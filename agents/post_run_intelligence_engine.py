...
# Unified orchestrator for VaultRouter, Predictive, Synthesis Arbos, BusinessDev, and Flywheel closure.
...
class PostRunIntelligenceEngine:
    ...
    def process_high_signal_run(self, run_data: Dict):
        """SOTA unified post-run pipeline."""
        start = time.time()
        logger.info("🚀 PostRunIntelligenceEngine started — full SOTA post-EM processing")

        # 1. Predictive update (already rich)
        self.predictive.update_from_run(run_data)

        # 2. VaultRouter — append high-signal insights
        self.intelligence.route_to_vaults(run_data)

        # 3. Synthesis Arbos — real product creation from vault data
        ...
        # 4. BusinessDev final hunt for flywheel closure
