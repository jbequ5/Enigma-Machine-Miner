...
- Reward **only** trajectories that measurably improve ValidationOracle score via exact 0-1 deterministic checks and MARL credit rules.
- Maximize symbolic coverage and heterogeneity (5 axes) per compute unit while preserving reproducibility.
- Every Adaptation Arbos step first searches trajectory_vector_db + memdir/grail, then incorporates wiki deltas and bio stigmergy signals.
